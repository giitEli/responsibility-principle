...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  const costOfPie = findPieCost(recipe);
  ...
  const revenue = totalRev(totalCost, pieQuantity, profitMargin);
}

const totalRev = (totalCost, pieQuantity, profitMargin) =>{
  const revenue = totalCost * (profitMargin || 1.2);
  console.log(`Sold ${pieQuantity} pies for $${revenue.toFixed(2)}!`);
  return revenue;
}

const findPieCost = recipe =>{
  return recipe.reduce((prev, current) => {
    return prev + current.cost;
  }, recipe[0].cost);
}

const combineIngredients = (recipe, pieType, pieQuantity) =>{
